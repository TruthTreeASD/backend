...
+function ($) {
  ...
  var Carousel = function (element, options) {
    ...
    this.options.pause == 'hover' && !('ontouchstart'
        in document.documentElement) && this.$element
...
+function ($) {
  ...
  function Plugin(option) {
    return this.each(function () {
      ...
      if (!data && options.toggle && /show|hide/.test(
          option)) {
...
+function ($) {
  ...
  function clearMenus(e) {
    ...
    $(toggle).each(function () {
      ...
      $parent.removeClass('open').trigger(
          $.Event('hidden.bs.dropdown', relatedTarget))
    })
  }
  ...
  Dropdown.prototype.keydown = function (e) {
    if (!/(38|40|27|32)/.test(e.which) || /input|textarea/i.test(
        e.target.tagName)) {
      return
    }
  ...
  Tooltip.prototype.init = function (type, element, options) {
    ...
    this.$viewport = this.options.viewport && $(
        $.isFunction(this.options.viewport) ? this.options.viewport.call(this,
            this.$element) : (this.options.viewport.selector
...
+function ($) {
  ...
  ScrollSpy.prototype.getScrollHeight = function () {
    return this.$scrollElement[0].scrollHeight || Math.max(
        this.$body[0].scrollHeight, document.documentElement.scrollHeight)
  }
